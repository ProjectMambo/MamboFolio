...
```dataviewjs
const FOLDER_PATH = "MamboFolio/gallery";
const container = dv.el('div', '');

// --- 1. QUICK ADD ---
const addBtn = dv.el('button', '＋ Quick Add Gallery');
...
addBtn.onclick = async () => {
    ...
    const title     = await qa.inputPrompt("Gallery Title");
    if (!title) return;
...
TABLE 
    ...
    date AS Date
FROM ""
WHERE file.folder = this.file.folder + "/gallery"
SORT file.name ASC
```